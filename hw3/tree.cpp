...
void order(double a[], int n)
{
  ...
  int l,g;
  split(a, n, a[0], g, l);
  if(l == n) order(a, g); //my splitter is the smallest item - order everything in front of me that isn't me
  else if(g == 0) order(a+l, n-l); //splitter is biggest  //order everything after me that isn't me
  else { //my splitter is somewhere in the middle
    order(a+l, n-l); //order those items less than me
    order(a, g); //order those items greater than me
  }
}
/*
int main() {
  double arr[100] = {10,50,40,20,50,40,30};
  double a1[3] = {10,20,40};
  double a2[3] = {10,40,30};
  double a3[3] = {20,10,40};
  double a4[3] = {50,40,30};
  assert(countIncludes(arr, 7, a1, 3) == 1);
  assert(countIncludes(arr, 7, a2, 3) == 2);
  assert(countIncludes(arr, 7, a3, 3) == 0);
  assert(countIncludes(arr, 7, a4, 3) == 3);



  double z[5] = {99,22,3,1,12};
  int g, l;
  order(z,5);
  //split(z, 5, z[0], g, l);
  //split(z, 4, z[0], g, l);
