...
bool IntelWeb::ingest(const std::string &telemetryFile) {
  ...
  ifstream fc(telemetryFile);
  int total = count(istreambuf_iterator<char>(fc), istreambuf_iterator<char>(), '\n');
  cerr << "Processing " << total << " items..." << endl;
  */
  ...
  while(getline(f, line)) {
    ...
    //TODO: REMOVE BEFORE SUBMIT
    /*
    m_objectsCount[destination]++;
    m_objectsCount[origin]++;
    */
    //cerr << "Processed " << context << " | " << origin << " | " << destination << endl;
    cerr << "#";
  }
  ...
  //TODO: REMOVE BEFORE SUBMIT
  /*
  cerr << "- Object Counts -" << endl;
  for(auto it = m_objectsCount.begin(); it != m_objectsCount.end(); it++) {
...
unsigned int IntelWeb::crawl(const std::vector<std::string> &indicators,
  ...
  while(!examineQueue.empty()) {
    ...
    examineQueue.pop();
    //cerr << "Examining " << obj << " count: " << prevalence(obj, minPrevalenceToBeGood) << endl;
